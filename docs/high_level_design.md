...
- PCなどの大画面では**2カラム（見開き）風**、モバイルなどの小画面（画面幅767px以下）では確実に**単一カラム（1段組み）**となるよう、CSSのマルチカラム（`column-width`）プロパティに viewport 幅からパディングを差し引いた値を設定し、表示を1カラムに強制します。また、縦書き文字の表示開始位置（縦位置）を常に上端（上側）に揃えるようにアライメントを設定します。
