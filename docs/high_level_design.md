...
- PCなどの大画面では**2カラム（見開き）風**、モバイルなどの小画面（画面幅767px以下）では確実に**単一カラム（1段組み）**となるよう、CSSのマルチカラム（`column-width`）プロパティに viewport 幅からパディングを差し引いた値を設定し、表示を1カラムに強制します。また、フレックス要素の高さ潰れによる二段化や空白バグを避けるため、スクロールコンテナをブロック要素にし、コンテンツ高さを100%に固定して上端に揃えます。さらに、親要素の `direction: rtl`（RTLスクロール制御用）が子要素のテキストフローに波及して文字進行が「下から上」に反転するのを防ぐため、テキスト表示要素自体の `direction` を `ltr` に固定し、日本語の伝統的な「上から下」方向の文字送り・アライメント・禁則処理を維持します。
