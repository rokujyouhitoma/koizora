...
- PCなどの大画面では**2カラム（見開き）風**、モバイルなどの小画面（画面幅767px以下）では確実に**単一カラム（1段組み）**となるよう、CSSのマルチカラム（`column-width`）プロパティに viewport 幅からパディングを差し引いた値を設定し、表示を1カラムに強制します。また、フレックス要素の高さ潰れによる二段化や空白バグを避けるため、スクロールコンテナをブロック要素にし、コンテンツ高さを100%に固定して上端に揃えます。
