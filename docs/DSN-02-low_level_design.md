...
* **対策**: `.reader-viewport` からフレックスレイアウト（`display: flex`, `justify-content`, `align-items`）を完全に撤廃し、絶対配置レイアウトに変更します。さらに、ヘッダー/フッターを絶対配置のオーバーレイ形式とし、読書用コンテンツの上下余白をCSS変数（`--reader-padding-top`, `--reader-padding-bottom`）として定義します。コンテナ自体の `left` および `right` を `0` として左右とも画面ぎりぎりまで広げ、コンテナ内部の padding は `0` にリセットしたうえで、安全余白パディングは子要素 `.reader-content` 側で持たせます。
  ```css
  .reader-viewport {
      position: absolute;
      top: 0;
