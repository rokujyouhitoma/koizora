...
- 画面の左右端に安定した静的余白を確保し、スクロール動作中も含めて文字の左右見切れを防ぐため、「ビューポート（閲覧窓）」の概念を採用します。スクロールコンテナである `.reader-viewport` 自体を画面の左右端から `var(--reader-padding-x)` だけ内側に引き込めて配置（`left: var(--reader-padding-x); right: var(--reader-padding-x);`）し、余白をコンテナの外側に静的に固定します。さらに、文字のグリッドがスクロールコンテナの物理的な境界（`overflow: hidden`）に密着して左端の文字（`vertical-rl` における行末のへん側など）がクリッピングされてしまう現象を防ぐため、安全パディングを設けます。ただし、スクロールコンテナ自体に `padding` を設定すると、ブラウザ（特に Chrome/Safari）がスクロール終端において左パディングを無視してスクロール可能領域を算出する既知の仕様・バグが存在するため、これを回避する**代替策（ワークアラウンド）**として、パディングをコンテナ（`.reader-viewport`）ではなく子要素のコンテンツブロック（`.reader-section`）自身に適用し（`padding: 0 var(--reader-viewport-padding-x)`）、コンテナ側は `padding: 0` にリセットします。このセクション内パディングを考慮した上で、横スクロールのページ送り幅がビューポートの物理幅（`clientWidth`）と完全に同期し、スクロール位置の累積ズレを完全に防ぐよう、カラム幅（`column-width`）を数学的に整合させます。具体的には、モバイル時は1カラム（`column-width: calc(100% - var(--reader-viewport-padding-x) * 2)`）、PC時は見開き2カラム（`column-width: calc(50% - var(--reader-viewport-padding-x) * 2)`）とし、カラム間隙間（`column-gap`）をパディングの2倍（`calc(var(--reader-viewport-padding-x) * 2)`）とします。これにより、各ページは左右に均等な内部安全パディングを確保した状態で美しくセンタリングされ、ブラウザのスクロール仕様に依存せず、累積的なレイアウトずれや見切れを完全に排除します。
