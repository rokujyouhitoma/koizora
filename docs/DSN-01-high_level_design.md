...
- 画面の左右端に安定した静的余白を確保し、スクロール動作中も含めて文字の左右見切れを防ぐため、「ビューポート（閲覧窓）」の概念を採用します。スクロールコンテナである `.reader-viewport` 自体を画面の左右端から `var(--reader-padding-x)` だけ内側に引き込めて配置（`left: var(--reader-padding-x); right: var(--reader-padding-x);`）し、余白をコンテナの外側に静的に固定します。さらに、文字のグリッドがスクロールコンテナの物理的な境界（`overflow: hidden`）に密着して左端の文字（`vertical-rl` における行末のへん側など）がクリッピングされてしまう現象を完全に排除するため、ビューポート自体の左右の内側に安全パディング（`--reader-viewport-padding-x`）を設けます。この内部パディングを考慮した上で、ページ送り（横スクロール）幅がビューポートの物理幅（`clientWidth`）と完全に同期するよう、カラム幅（`column-width`）とカラム隙間（`column-gap`）を数学的に整合させます。モバイル時は1カラム（`column-width: 100%; column-gap: calc(var(--reader-viewport-padding-x) * 2)`）、PC時は2カラム見開き（`column-width: calc(50% - var(--reader-viewport-padding-x)); column-gap: calc(var(--reader-viewport-padding-x) * 2)`）に設定することにより、各ページは左右に均等な内部安全パディングを確保して美しくセンタリングされ、累積的なレイアウトずれや見切れを完全に排除します。
