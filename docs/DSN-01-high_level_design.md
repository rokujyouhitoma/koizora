...
- 画面の左右端に安定した静的余白を確保し、スクロール動作中も含めて文字の左右見切れを防ぐため、「ビューポート（閲覧窓）」の概念を採用します。スクロールコンテナである `.reader-viewport` 自体を画面の左右端から `var(--reader-padding-x)` だけ内側に引き込めて配置（`left: var(--reader-padding-x); right: var(--reader-padding-x);`）し、余白をコンテナの外側に静的に固定します。これにより、各セクションの横幅は余白の影響を受けず完璧にビューポート幅の整数倍（モバイル時は 100%、PC時は見開き 2 カラム）として同期され、累積的なレイアウトずれや見切れを完全に排除します。
