...
- ページ境界（段送り位置）付近において、見出しや段落が中途半端に分割されて文字の左右が見切れたり、縦方向に欠けたりするのを防ぐため、段落（`<p>`）および見出し（`<h1>`-`<h5>`）要素に対し改段・改ページ抑止（`break-inside: avoid`）を適用します。これにより、ページ境界にかかる文章は要素単位で自動的に次ページに送られ、美しいレイアウトを維持します。
